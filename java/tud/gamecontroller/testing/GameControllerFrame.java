...
import tud.gamecontroller.testing.PlayerTableModel.PlayerRecord;
...
public class GameControllerFrame extends JFrame {
	...
	private Game<Term, GameState> game;
	

	public GameControllerFrame(GARunner gaRunner) {
		super();
		this.gaRunner = gaRunner;
		game = gaRunner.getGame();
		initialize();
	}
	...
	private void startGame() {
		gaRunner.setMatchID(jMatchIDTextField.getText());
		gaRunner.setStartclock(((Integer)jStartclockComboBox.getSelectedItem()).intValue());
		gaRunner.setPlayclock(((Integer)jPlayclockComboBox.getSelectedItem()).intValue());
		...
		gaRunner.setPlayerInfos(players);
		...
		gameThread=new Thread(){
			public void run(){
				try {
					...
					gaRunner.getLogger().log(Level.INFO, "game interrupted!");
				}
